...
\begin{questions}
    ...
    \begin{parts}
        ...
        \begin{solution}
            ...
            \paragraph{negation} For any $y$, there is no $x$ smaller than $y$
            such that $f(x)$ is smaller than $f(y)$.  $\forall x$, $\nexists y$
            such that if $x < y$ then $f(x) < f(y)$.  Or, $\forall x,\; \exists
            y\;$ such that if $x <y $ then $f(x) \ge f(y)$.
    ...
    \begin{parts}
        ...
        \begin{solution}
            ...
            That is $ax^3 + cx = 0 \implies a = 0,c =0$. See David's explanation
            on group.
            Similarly, for $f$ to be odd, $-ax^3 + bx^2 -cx+d = -ax^3 -bx^2 -cx-d$.
            That is $bx^2 +d = 0 \implies b = 0, d= 0$.
        \end{solution}
